...
function resolveGoogleRedirectURL(event: H3Event) {
  const config = useRuntimeConfig(event)
  const configured = String(config.oauth.google.redirectURL || '').trim()
  if (configured) {
    return configured
  }

  const siteUrl = String(config.public.siteUrl || '').replace(/\/$/, '')
  return siteUrl ? `${siteUrl}/auth/google` : ''
}

export default defineEventHandler((event) => {
  const redirectURL = resolveGoogleRedirectURL(event)
